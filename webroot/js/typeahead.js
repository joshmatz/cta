...
ajaxSuggestFbml.prototype.onajaxdone = function(data) {
	...
	if(this.get_norm_typed() == data.fortext) {
		if(data.fortext == null)
			alert('for text empty');
		else
			alert(data.fortext);
		if(data.results == null)
			alert('Results is empty, first');
		else
			alert(data.results);
...
ajaxSuggestFbml.prototype.draw_results = function(results, typed) {
	...
	if(results == null) {
		alert('Bottom Results is empty');
		return;
	}
	for( var i = 0; i < this.results.length; i++ ) {
		this.item = document.createElement('div').setClassName('suggest_suggestion');
		...
		var begins = this.results[i].toLowerCase().indexOf(typed);
		if (begins == -1) {
			this.span_item = document.createElement('span').setTextValue(results[i]);
			this.item.appendChild(span_item);
		} else {
			if (begins > 0) {
				this.item.appendChild(document.createElement('span').setTextValue(results[i].substring(0, begins)));
			}
			this.em_item = document.createElement('em').setTextValue(results[i].substring(begins, begins + typed.length));
			this.item.appendChild(this.em_item);

			var span_item = document.createElement('span').setTextValue(results[i].substring(begins + typed.length));
			this.item.appendChild(this.span_item);
		}
...
ajaxSuggestFbml.prototype.send_ajrequest = function(val) {
	...
	this.cache[val].curRequest.post((this.options.ajaxUrl + val));
};
